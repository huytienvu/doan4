...
export default function CategoryManagement() {
  ...
  const [formData, setFormData] = useState({ ten: '', mota: '', status: "Hiển thị" });
  ...
  const handleAdd = () => {
    ...
    setFormData({ ten: '', mota: '', status: "Hiển thị" });
    setError('');
    setIsModalOpen(true);
  };
  ...
  const handleEdit = (category) => {
    ...
    setFormData({ ten: category.ten, mota: category.mota || '', status: category.status });
    setError('');
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingCategory) {
        // Cập nhật
        await UpdateMovie(editingCategory.id, formData);
        setCategories(categories.map(cat =>
          cat.id === editingCategory.id
            ? { ...cat, ten: formData.ten, mota: formData.mota }
            : cat
        ));
        loadCategories();
        ...
        // Thêm mới
        const newCategory = await CreateMovie(formData);
        loadCategories();
      ...
      setError('Lỗi khi lưu danh mục. Vui lòng thử lại!');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-500 mt-1">Quản lý và tổ chức danh mục phim của bạn</p>
              </div>
            </div>
            <button
              onClick={handleAdd}
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          {!loading && !error && categories.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mt-2">Bắt đầu bằng cách thêm danh mục mới</p>
            </div>
          )}
        </div>
